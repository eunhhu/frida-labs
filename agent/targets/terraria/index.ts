...
rpc.exports = defineInstrument({
  info: read({
    ...
    category: "Start here",
    doc: "Runtime, offline safety boundary, and cleanup limitations",
  }, () => ({
      game: "Terraria",
  ...
  state: read({
    ...
    category: "Start here",
    doc: "Lazy initialization, enabled feature ids, and clean state",
  }, () => {
  ...
  actions: {
      ...
      args: [
        field.select("id", {
          ...
          options: [
            ...
            { label: "Infinite mana", value: "mana" },
            { label: "Infinite breath", value: "breath" },
            { label: "No knockback", value: "nokb" },
            { label: "Flight", value: "fly" },
            { label: "Environment immunity", value: "env" },
            { label: "Max summons", value: "maxsummon" },
            { label: "Fast use", value: "fastuse" },
            { label: "Map teleport", value: "maptp" },
          ],
        }),
        field.checkbox("on", { label: "Enabled" }),
        field.offline({ optional: true }),
      ],
      doc: "Enabling requires an owned offline session and starts the owned timer lazily",
      returns: "scalar",
      status: "modState",
    }, (id: FeatureId, on: boolean, offlineConfirmed?: boolean) => {
      if (on) requireOffline(offlineConfirmed, "set");
      return activeInstrument().setFeature(id, !!on);
    }),
    cmd: control({
      label: "Terraria command",
      category: "Player",
      args: [
        field.text("raw", { label: "Command", placeholder: "heal | give 757 1 | time day" }),
        field.offline({ optional: true }),
      ],
      doc: "Mutating commands require an owned offline session; one-shot effects may persist",
      returns: "scalar",
    }, (raw: string, offlineConfirmed?: boolean) => {
    ...
    playerSnapshot: read({
      label: "Live player values",
      category: "Player",
      doc: "HP, mana, breath, movement, environment, summon, position, world, and apply-tick readback",
  ...
  reset: control({
    label: "Stop continuous features",
    category: "Start here",
    doc: "Disable features and remove the timer/chat hook",
    status: "modState",
  }, disposeInstrument),
  dispose: control({
