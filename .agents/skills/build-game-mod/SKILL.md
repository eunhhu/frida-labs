...
3. Run preflight and a persistent generic-probe recon on the exact selected
   device before scaffolding. Build a subsystem coverage map from evidence.
...
6. Provide real `modInfo`, `modHelp`, `modState`, domain actions, and cleanup.
   ...
   by default. Do not claim content creation without a discovered creation path.
...
8. Write `agent/targets/<slug>/README.md` and bounded receipts under
   `artifacts/<slug>/`.
