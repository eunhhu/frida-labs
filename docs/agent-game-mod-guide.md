...
2. A descriptor-driven game-specific Instrument in `flab run <slug>` (also
   reachable through `flab tui <slug>`) with
   input fields, checkboxes, sliders, selects, and a linked live-state pane.
3. An opt-in advanced game-specific console in `flab run <slug> --console`
   with generated help and tab completion over the same actions.
...
The control panel, advanced console, and AI protocol are three views generated
from one `defineInstrument()` declaration.
...
Declare every callable once with `defineInstrument()`. It generates both live
`rpc.exports` handlers and the `__describe()` inventory; never maintain those
two surfaces by hand. Example:

```ts
rpc.exports = defineInstrument({
  info: read({ label: "About", category: "Start here" }, () => modInfo()),
  state: read({ label: "Live state", category: "Start here" }, () => modState()),
  actions: {
    playerSetSpeed: write({
      label: "Set movement speed",
      category: "Player",
      args: [field.slider("multiplier", {
        label: "Movement speed", min: 0.5, max: 3, step: 0.25, default: 1,
      })],
      doc: "Set local movement; reset restores the captured value.",
      status: "modState",
    }, (multiplier: number) => playerSetSpeed(multiplier)),
  },
  reset: control({ label: "Reset all", category: "Start here", status: "modState" }, resetAll),
});
```
...
```ts
aimAssistSet: control({
  label: "Toggle bounded aim assistance",
  category: "Assist",
  args: [
    field.checkbox("enabled", { label: "Aim assistance" }),
    field.offline(),
  ],
  doc: "Requires verified hostile/visibility/input paths; reset stops its timer.",
  status: "aimAssistStatus",
}, (enabled: boolean, offlineConfirmed: boolean) =>
  aimAssistSet(enabled, offlineConfirmed)),
```

`field.text/json/address/pattern`, `field.checkbox`, `field.slider`, and
`field.select` cover input, checkbox, slider, and select widgets. Use bounded
`{label,value}` options for selects. Defaults must be valid action arguments. The TUI
renders at most three fields at once and turns the action's `status` link into
descriptor `statusAction`; otherwise it uses zero-argument `modState` for the
live-state pane. `.help`, `/help`, and `:help`
are generated by flab from the same declaration. Do not implement a per-target
`modHelp()` or hand-maintained `rpc.exports`/descriptor pair.
...
- Mark writes, hooks, and controls truthfully. Use DSL option `status` for a
...
Those commands open the human dashboard. For the advanced console, add
`--console`:

```sh
flab run <slug> --device <selector> --console
flab run <slug> --host <host:port> --console
```
...
- [ ] `modInfo`, `modState`, and cleanup are real, not stubs; one
      `defineInstrument()` declaration generates RPC, descriptors, and help,
      with no parallel arrays or target-local `modHelp()` boilerplate.
